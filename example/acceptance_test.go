	...
	tests := []struct {
		...
		},
		{
			...
			args: []string{
				...
				"--go-type", "text=github.com/mbark/pggen/example/custom_types/mytype.String",
				"--go-type", "int8=github.com/mbark/pggen/example/custom_types.CustomInt",
				"--go-type", "my_int=int",
				"--go-type", "_my_int=[]int",
			},
		},
		...
		},
		{
			name: "example/jsonb",
			args: []string{
				"--schema-glob", "example/jsonb/schema.sql",
				"--query-glob", "example/jsonb/query.sql",
			},
		},
	}
	if *update {
